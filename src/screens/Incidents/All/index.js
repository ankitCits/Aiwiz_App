...
import {INCIDENT_URL, TODAY, YESTERDAY} from '../../../config';
...
export default class AllIncidents extends Component {
  ...
  fetchTeamsData = async fetch => {
    ...
    var config = {
      method: 'get',
      url: `${INCIDENT_URL}/incidents/export/?start_time=${this.state.startTime}&end_time=${this.state.endTime}&type=json&owner_id=${this.state.ownerId}`,
      headers: {
    ...
    axios(config)
      .then(response => {
        ...
        console.log('DATA_LENGTH>', response.data.incidents.length);
      })
      .catch(error => {
        this.setState({isFetching: false});
